...
def make_post_title(keyword: str) -> str:
    # 다양한 자연스러운 베트남어 제목 패턴
    patterns = [
        f"{keyword} cho iPhone — Ứng dụng tốt nhất bạn nên thử",
        f"Các ứng dụng {keyword} phổ biến cho người dùng iPhone",
        f"Khám phá {keyword} trên iOS — Ứng dụng nên tải ngay",
        f"{keyword} iOS — Lựa chọn hàng đầu cho iPhone",
        f"{keyword} — Ứng dụng hữu ích cho iPhone của bạn"
    ]
...
def make_post_labels(sheet_row: list) -> list:
    ...
    labels = ["Ứng dụng", "iPhone"]  # 베트남어 라벨
    if label_val:
        labels.append(label_val)
    return labels

# =============== 앱스토어 검색 (iTunes Search API, 베트남) ===============
def search_app_store_ids(keyword, limit=20, country="vn", eng_keyword=""):
    ...
    def fetch(term):
        ...
        try:
            ...
            print("[iTunes API 오류]", e)  # 한글로 에러 출력
            return []
    ...
    if len(apps) < 7:
        apps += fetch(f"{keyword} ứng dụng")  # 'uygulama' → 베트남어 'ứng dụng'
...
def fetch_app_detail(app_id: str, country="vn"):
    ...
    name = f"Ứng dụng {app_id}"  # 기본 앱 이름
    desc_html, images = "", []
    ...
    try:
        ...
        print(f"[앱 상세 수집 실패] id={app_id}, error={e}")  # 한글 에러 로그
...
def upload_to_drive(file_path, file_name):
    try:
        ...
        print(f"에러: 구글드라이브 업로드 실패: {e}")  # 한글 로그
        return ""

# =============== 썸네일 생성 + 로그 + 업로드 (베트남 버전) ===============
...
def build_intro_block(title: str, keyword: str) -> str:
    intro_groups = [
        [
            f"Ngày nay, điện thoại thông minh không chỉ là phương tiện liên lạc mà còn là công cụ thiết yếu trong cuộc sống hiện đại.",
            f"Với một thiết bị trong tay, bạn có thể tận dụng các tính năng như 『{keyword}』 một cách nhanh chóng và tiện lợi.",
            f"Trong cuộc sống hàng ngày, các ứng dụng 『{keyword}』 trở nên không thể thiếu để mang lại sự tiện nghi.",
            f"Chủ đề như 『{title}』 luôn thu hút sự quan tâm của nhiều người dùng công nghệ.",
            f"Với sự phát triển của smartphone, việc sử dụng ứng dụng liên quan đến 『{keyword}』 ngày càng phổ biến.",
            f"Mọi người đều có thể trải nghiệm 『{keyword}』 dễ dàng thông qua các ứng dụng chuyên dụng."
        ],
        [
            f"Có rất nhiều ứng dụng khác nhau và các lựa chọn cho 『{keyword}』 ngày càng đa dạng.",
            f"Việc tìm kiếm 『{title}』 cho thấy chủ đề này đang rất thịnh hành.",
            f"Công việc, học tập, giải trí và cả 『{keyword}』 đều có thể được tối ưu hóa bằng ứng dụng.",
            f"Ứng dụng giúp tiết kiệm thời gian và làm cho cuộc sống hiệu quả hơn.",
            f"Ứng dụng 『{keyword}』 mang đến trải nghiệm mới và sự tiện lợi cho người dùng.",
            f"Các ứng dụng mới liên quan đến 『{keyword}』 xuất hiện hàng ngày, tăng thêm nhiều lựa chọn."
        ],
        [
            f"Từ các ứng dụng về năng suất đến giải trí, tất cả đều liên quan đến 『{keyword}』.",
            f"『{title}』 là một trong những danh mục phổ biến nhất đối với người dùng.",
            f"Cũng như trò chơi và streaming, các ứng dụng 『{keyword}』 làm cho thời gian rảnh thêm thú vị.",
            f"Mua sắm, tài chính và giao thông đều dựa vào ứng dụng — 『{keyword}』 cũng không ngoại lệ.",
            f"Nhiều ứng dụng cho phép quản lý nội dung như ảnh, video và 『{keyword}』 một cách đơn giản.",
            f"Ứng dụng 『{keyword}』 ngày càng được ưa chuộng cùng với các ứng dụng liên lạc."
        ],
        [
            f"Nhờ đó, ứng dụng 『{keyword}』 vượt ra ngoài chức năng cơ bản và thay đổi cuộc sống người dùng.",
            f"Với 『{title}』, bạn có thể nâng cao chất lượng cuộc sống hàng ngày.",
            f"Khi cần, chỉ cần mở ứng dụng 『{keyword}』 để sử dụng tính năng mong muốn.",
            f"Không chỉ tiện lợi, ứng dụng 『{keyword}』 còn mang đến trải nghiệm mới mẻ.",
            f"Nhiều người đã tận dụng ứng dụng 『{keyword}』 để có một cuộc sống thông minh hơn.",
            f"Một ứng dụng 『{keyword}』 có thể thay đổi hoàn toàn phong cách sống của bạn."
        ]
    ]
...
def build_ending_block(title: str, keyword: str) -> str:
    end_groups = [
        [
            ...
            f"Những lựa chọn 『{title}』 được trình bày có thể giúp bạn đưa ra quyết định thông minh hơn.",
            f"Nếu các ứng dụng 『{title}』 trở thành công cụ không thể thiếu, chúng tôi sẽ rất vui.",
            f"Đối với những ai quan tâm đến 『{title}』, tóm tắt này là một tham khảo giá trị.",
            f"Khi khám phá nhiều ứng dụng liên quan đến 『{keyword}』, việc sử dụng smartphone của bạn sẽ đầy đủ hơn."
        ],
        [
            f"Chúng tôi giới thiệu các tính năng và ưu điểm của từng app để giúp bạn chọn lựa 『{keyword}』.",
            f"So sánh điểm mạnh và yếu giúp bạn dễ dàng quyết định nên tải app 『{title}』 nào.",
            f"Dựa trên tóm tắt này, bạn có thể tìm thấy ứng dụng 『{keyword}』 lý tưởng.",
            f"Thông tin được tổng hợp ở đây rất thực tế và có thể làm hướng dẫn tham khảo.",
            f"Khi chọn app 『{keyword}』, bài viết này sẽ là người bạn đồng hành đáng tin cậy.",
            f"So sánh nhiều ứng dụng chắc chắn giúp bạn đưa ra quyết định sáng suốt hơn."
        ],
        [
            "Chúng tôi sẽ tiếp tục mang đến những thông tin và cập nhật hữu ích về ứng dụng.",
            f"Trong tương lai, nhiều nội dung về 『{keyword}』 và app gợi ý sẽ được đăng tải tại đây.",
            "Ý kiến của độc giả rất quan trọng để chúng tôi cung cấp bài viết toàn diện hơn.",
            "Sắp tới, chúng tôi sẽ giới thiệu các ứng dụng và tính năng nổi bật mới.",
            "Chúng tôi sẽ tiếp tục cập nhật thông tin thực tế để hỗ trợ công việc và cuộc sống.",
            f"Các chủ đề phổ biến như 『{title}』 sẽ tiếp tục được khai thác trên blog."
        ],
        [
            "Bình luận và lượt thích là nguồn động lực lớn — hãy tham gia thoải mái!",
            "Nếu có thắc mắc hoặc gợi ý, hãy chia sẻ trong phần bình luận để chúng tôi cải thiện.",
            "Phản hồi của bạn rất quan trọng để tạo ra nội dung hữu ích hơn.",
            "Hãy ủng hộ bằng like hoặc bình luận để chúng tôi tiếp tục mang thông tin chất lượng.",
            "Nếu có ứng dụng nào quan tâm, hãy đề xuất trong phần bình luận.",
            f"Ý kiến của bạn về 『{keyword}』 rất được hoan nghênh — hãy chia sẻ ngay!"
        ]
    ]
...
if __name__ == "__main__":
    try:
        # 1) 시트9에서 대상 행/데이터
        target_row, row = pick_target_row(ws9)
        ...
        if not apps:
            ...
            ws9.update_cell(target_row, 7, "")        # G열 = URL 비움
            sheet_append_log(ws9, target_row, "시트 기록 완료: E='OK', G='' (검색 결과 없음)")
            raise SystemExit(0)

        # ✅ 앱이 3개 미만일 경우
        if len(apps) < 3:
            sheet_append_log(ws9, target_row, "앱 개수가 3개 미만 → 완료 처리")
            ws9.update_cell(target_row, 5, "OK")      # E열 완료
            ws9.update_cell(target_row, 7, "")        # G열 = URL 비움
        ...
        # ✅ 앱이 충분히 있을 경우 → 로그 기록
        sheet_append_log(ws9, target_row, f"앱 ID={[(a['id'], a['name']) for a in apps]}")
        ...
        # ✅ 목차 블록 추가
        html_full += """
        <div class="mbtTOC"><button>Nội Dung</button>
...
"""
        ...
        for j, app in enumerate(apps, 1):
            ...
            try:
                ...
                detail = fetch_app_detail(app["id"], country="vn")
                app_url = detail["url"]
        ...
        # 10) 업로드
        try:
            labels = make_post_labels(row)  # ["Ứng Dụng", "iPhone", B열 값]
        ...
        # 11) 시트 기록
        ws9.update_cell(target_row, 5, "OK")      # E열 완료
        ws9.update_cell(target_row, 7, post_url)  # G열 = URL
