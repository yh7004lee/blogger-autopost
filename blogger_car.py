...
def extract_images_from_section(page, section_name, section_index, max_count, photo_url):
    return []
...
def main():
    ...
    log_step(f"2단계: 대상 행={target_row}, 차량명={car_name}")
    ...
    try:
        ...
        async def run_browser():
            nonlocal summary_items, extracted_specs, exterior_urls, interior_urls, web_image_urls
            async with async_playwright() as p:
                ...
                price_patterns = [r"(\d{1,3},\d{3}\s*~\s*\d{1,3},\d{3}\s*만원)", r"(\d{1,3}\s*~\s*\d{1,3}\s*만원)", r"(\d[\d,]*\s*만원)"]
                ...
                try:
                    ...
                    for i in range(row_count):
                        row = rows.nth(i)
                        if await row.locator("th").count() > 0 and await row.locator("td").count() > 0:
                            raw_k = await row.locator("th").first.inner_text()
                            k = raw_k.replace("정보확인 내용 열고 닫기", "").strip()
                            raw_v = await row.locator("td").first.inner_text()
                            v = " ".join(raw_v.split())
                            if "배기량" in k:
